											...
											<div class="panel panel-plain">
									    		...
									    		<div class="panel-body" style="padding-top: 0px;">
										    		<div class="row">
										            	<div class="col-sm-12">
											    			<div id="toolbar">
														    	<perm:tag permPath="/manageAdminRole/addManageAdminRole" >
														    	<button type="button" onclick="to_add()" class="btn btn-primary">
															        <i class="glyphicon glyphicon-plus"></i>添加
															    </button>
																</perm:tag>
																<perm:tag permPath="/manageAdminRole/removeAllManageAdminRole" >
															...
															<table id="table" data-toggle="table"
														        ...
														        data-url="${ctx}/manageAdminRole/getManageAdminRoleList"
    ...
    <script type="text/javascript">
	    ...
	    function actionFormatter(value, row, index) {
	        return [
	            ...
	            '<perm:tag permPath="/manageAdminRole/updateManageAdminRole" >',
	            '<a class="edit" href="javascript:void(0)" title="编辑">',
	            '<i class="glyphicon glyphicon-edit"></i>',
	            '</a> | ',
	            '</perm:tag>',
	            '<perm:tag permPath="/manageAdminRole/removeManageAdminRole" >',
	            '<a class="remove" href="javascript:void(0)" title="删除">',
	            '<i class="glyphicon glyphicon-remove"></i>',
	            '</a>',
	            '</perm:tag>',
	            '<perm:tag permPath="/manageAdminRole/showColumn" >',
	            ' | <a class="showColumn" href="javascript:void(0)" title="授权菜单">',
    	...
    	//操作事件
	    window.actionEvents = {
    		'click .showColumn': function (e, value, row, index) {
    	    	showColumn(row.roleId);
    	    },
...
//     	        maxmin: true, //开启最大化最小化按钮
    	        area: ['80%', '80%'],
    	        content: '${ctx}/manageAdminRole/showColumn?roleId='+roleId
    	      });
    	}
    	function to_add(){
    		layer.open({
				title : "添加",
			  	...
			  	zIndex:900,
			  	 area: ['60%', '80%'],
			  	shade: false,
			  	content: '${ctx}/manageAdminRole/toAdd'
			 });
    	}
    	function toUpdate(id){
    		layer.open({
				title : "编辑",
			  	...
			  	area: ['60%', '80%'],
			  	shade: false,
			  	content: '${ctx}/manageAdminRole/toUpdate?roleId='+id 
			 });
    	}
    	function del(roleId){
   		   if (roleId != ""){ 
   				$.post("<c:url value='/manageAdminRole/removeManageAdminRole'/>",
	        	{
					roleId	:roleId,
					ranNum:Math.random()
				},
    	...
    	function del_all(){
   	    	...
   	    	if(ids && ids!=''){
	    		sys.confirm('确认全部删除',
	    				...
	    				'取消',
	    			function(){
	    	    	$.post("<c:url value='/manageAdminRole/removeAllManageAdminRole'/>",
	            	{
